import os
import re

# Standardized Physical Root-Relative Links
HOME_URL = "/index.html"
CATALOG_URL = "/products/index.html"
ANCHOR_BASE = "/index.html#"

FILES = [
    'index.html',
    'products/index.html',
    'products/ai-optical-sorter/index.html',
    'products/film-flexible-sorter/index.html',
    'products/hyperspectral-material-sorter/index.html',
    'products/parallel-robot-sorter/index.html',
    'products/solid-waste-line/index.html',
    'products/textile-sorter/index.html'
]

PRODUCT_SLUGS = [
    'ai-optical-sorter',
    'film-flexible-sorter',
    'hyperspectral-material-sorter',
    'parallel-robot-sorter',
    'solid-waste-line',
    'textile-sorter'
]

ANCHORS = ['markets', 'solutions', 'products', 'technology', 'services', 'contact']

def perform_precision_navigation_repair(file_path):
    if not os.path.exists(file_path):
        return
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # Step 1: index.html Entry Points (Product Cards)
    # Fix card links to be explicit index.html files
    for slug in PRODUCT_SLUGS:
        # Match href="/products/xxx/" or variants, ensure they point to physical index.html
        pattern = r'href="[^"]*?products/' + slug + r'/(?:index\.html)?"'
        content = re.sub(pattern, f'href="/products/{slug}/index.html"', content)

    # Step 2: Global Navigation Standardization for Sub-pages
    if file_path != 'index.html':
        # 2a. Home & Logo -> /index.html
        # Target specific patterns that represent home/logo links
        content = re.sub(r'href="(?:/|index\.html|(?:\.\./)+index\.html|#top|https://www\.fdlsorterai\.com/?)"(?=[^>]*?(?:logo|brand|Home|首页))', f'href="{HOME_URL}"', content)
        
        # 2b. Standard Nav Anchors -> /index.html#anchor
        for anchor in ANCHORS:
            if anchor == 'products':
                 # "Products/Equipment Center" menu item in sub-pages goes to Catalog Index
                 content = re.sub(r'href="[^"]*?#products"', f'href="{CATALOG_URL}"', content)
            else:
                 pattern = r'href="[^"]*?#' + anchor + r'"'
                 content = re.sub(pattern, f'href="{ANCHOR_BASE}{anchor}"', content)

    # Step 3: Page Specific Fixes (Related Products & Center button)
    if file_path.startswith('products/'):
        # "Back to Product Center" buttons
        content = re.sub(r'href="(?:/products/|(?:\.\./)*index\.html|index\.html|https://www\.fdlsorterai\.com/products/index\.html)"(?=[^>]*?(?:返回设备中心|Product Center|Equipment Center|设备中心))', f'href="{CATALOG_URL}"', content)
        
        # Related Product Cards at the bottom of detail pages
        for slug in PRODUCT_SLUGS:
            pattern = r'href="[^"]*?products/' + slug + r'/(?:index\.html)?"'
            content = re.sub(pattern, f'href="/products/{slug}/index.html"', content)

    # Step 4: Final Sanitization
    # Fix nested hrefs or double slashes
    content = re.sub(r'href="href="([^"]+)"\s*"', r'href="\1"', content)
    content = content.replace('href="//', 'href="/')
    # Remove control characters
    content = "".join(ch for ch in content if ch.isprintable() or ch in '\n\r\t')

    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Precise Repair Applied: {file_path}")

if __name__ == "__main__":
    for f in FILES:
        perform_precision_navigation_repair(f)
